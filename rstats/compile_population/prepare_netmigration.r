...
d19 <- read.csv("csv/stats-per-county/co-est2019-alldata.csv") %>%
    ...
    select(FIPS, starts_with("RNETMIG"))
d19 %>% nrow()
...
d10 <- d10 %>% group_by(FIPS) %>%
    summarize_at(vars(starts_with("RNETMIG")), mean) %>%
    ungroup()
...
names(data) <- gsub("RNETMIG", "", names(data))

data <- data %>%
    gather("YEAR", "RNETMIG", 2:ncol(data))

nrow(data)

write.csv(data, "csv/stats-per-county/counties_netmigration.csv")
