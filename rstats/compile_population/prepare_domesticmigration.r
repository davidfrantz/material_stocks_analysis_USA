...
d19 <- read.csv("csv/stats-per-county/co-est2019-alldata.csv") %>%
    ...
    select(FIPS, starts_with("RDOMESTICMIG"))
d19 %>% nrow()
...
d10 <- d10 %>% group_by(FIPS) %>%
    summarize_at(vars(starts_with("RDOMESTICMIG")), mean) %>%
    ungroup()
...
names(data) <- gsub("RDOMESTICMIG", "", names(data))

data <- data %>%
    gather("YEAR", "RDOMESTICMIG", 2:ncol(data))

nrow(data)

write.csv(data, "csv/stats-per-county/counties_domesticmigration.csv")
